...
@NoArgsConstructor
@Schema(
    title = "Reboot an LXC container on Proxmox VE",
    description = "Sends a reboot command to an LXC container and waits for the Proxmox task to complete."
)
@Plugin(
    examples = {
        @Example(
            title = "Reboot a container",
            full = true,
            code = """
                id: reboot_container
...
public class Reboot extends AbstractTask<AbstractTask.Output> {
    ...
    public AbstractTask.Output run(RunContext runContext) throws Exception {
        ...
        try (var client = createClient(runContext)) {
            ...
            logger.info("Rebooting container {} (vmid={})", rVmName, vmid);
            var upid = client.postAndWait("/nodes/" + URLEncoder.encode(rNode, StandardCharsets.UTF_8) + "/lxc/" + vmid + "/status/reboot", null);
            logger.info("Container {} rebooted", rVmName);
