...
Optionally set `verifySsl` (default `true`; set to `false` only for trusted networks with self-signed Proxmox certificates). Store secrets in [secrets](https://kestra.io/docs/concepts/secret) and set connection properties on each task.
